...
export default function VetAdvertise() {
  ...
  const handlePayConfirm = async (gateway) => {
    ...
    const payment = await entities.Payment.create({
      ...
      payer_email: form.contact_email,
    });
    // Update subscription with gateway + payment id
          ...
          <div className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl p-6 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div><Label className="text-xs">Email *</Label><Input required type="email" value={form.contact_email} onChange={f('contact_email')} className="rounded-xl mt-1" /></div>
              </div>
              ...
              <Button type="submit" className="w-full rounded-xl h-11 font-bold bg-primary">
                Proceed to Payment — SAR {(ANNUAL_FEE * 1.15).toFixed(2)}
              </Button>
            </form>
          </div>
        </div>
