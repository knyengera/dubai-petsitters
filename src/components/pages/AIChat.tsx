...
import { Bot, Send, User, AlertTriangle, Stethoscope, Heart, Zap } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-[calc(100vh-12rem)] bg-background">
      ...
      <div className="bg-orange-50 dark:bg-orange-900/20 border-b border-orange-200 dark:border-orange-800 px-4 py-2 flex items-center gap-2 shrink-0">
        ...
        <p className="text-xs text-orange-700 dark:text-orange-400">
          {t('AI guidance only — not a substitute for professional veterinary care', 'إرشادات ذكاء اصطناعي فقط — لا تغني عن الرعاية البيطرية المتخصصة')}
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center text-center pt-8 pb-4">
            <div className="w-20 h-20 bg-violet-100 dark:bg-violet-900/30 rounded-3xl flex items-center justify-center mb-4">
              <Bot className="w-10 h-10 text-violet-600" />
            </div>
            <h2 className="font-heading text-xl font-bold text-foreground mb-2">
              {t('How can I help your pet today?', 'كيف يمكنني مساعدة حيوانك اليوم؟')}
            </h2>
            <p className="text-muted-foreground text-sm max-w-xs mb-6">
              {t('Ask about symptoms, vaccinations, nutrition, or emergency care — in Arabic or English.', 'اسأل عن الأعراض أو التطعيمات أو التغذية أو الرعاية الطارئة — بالعربية أو الإنجليزية.')}
            </p>
        ...
        <AnimatePresence>
          {messages.map((msg, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              {msg.role === 'assistant' && (
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-1 ${isEmergency(msg.content) ? 'bg-red-500' : 'bg-violet-600'}`}>
                  {isEmergency(msg.content) ? <AlertTriangle className="w-4 h-4 text-white" /> : <Bot className="w-4 h-4 text-white" />}
                </div>
              )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
            ...
            <div className="bg-card border border-border rounded-2xl rounded-es-sm px-4 py-3">
              <div className="flex gap-1.5">
                {[0, 1, 2].map(i => <div key={i} className="w-2 h-2 bg-muted-foreground/50 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="px-4 py-4 border-t border-border bg-background shrink-0">
        <div className="flex gap-2 max-w-3xl mx-auto">
          <Input
            ...
            placeholder={t('Describe your pet\'s symptoms...', 'صف أعراض حيوانك الأليف...')}
            className="rounded-2xl flex-1"
            disabled={loading}
          />
          <Button onClick={() => sendMessage()} disabled={loading || !input.trim()} size="icon" className="rounded-2xl w-12 h-12 shrink-0">
